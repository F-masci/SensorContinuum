...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o proximity-fog-hub ./cmd/proximity-fog-hub
...
COPY --from=builder /app/proximity-fog-hub .

# Esegui il binario
ENTRYPOINT ["/app/proximity-fog-hub"]