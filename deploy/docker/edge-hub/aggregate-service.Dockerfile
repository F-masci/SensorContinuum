...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o edge-hub-aggregate ./cmd/edge-hub/aggregate-service.go
...
COPY --from=builder /app/edge-hub-aggregate .

# Esegui il binario
ENTRYPOINT ["/app/edge-hub-aggregate"]