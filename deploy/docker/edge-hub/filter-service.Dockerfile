...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o edge-hub-filter ./cmd/edge-hub/filter-service.go
...
COPY --from=builder /app/edge-hub-filter .

# Esegui il binario
ENTRYPOINT ["/app/edge-hub-filter"]