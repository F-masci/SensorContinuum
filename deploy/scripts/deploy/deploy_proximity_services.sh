...
for i in {1..20}; do
  docker exec -it macrozone-hub-${EDGE_MACROZONE}-cache pg_isready -U admin -d sensorcontinuum && break
  sleep 2
done
...
sudo ./"$DELAY_FILENAME" apply --delay "${NETWORK_DELAY:-30ms}" --jitter "${NETWORK_JITTER:-15ms}"
